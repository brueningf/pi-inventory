...
<div class="w-full flex flex-wrap my-6">
    @foreach ($items as $item)
        <div class="w-1/5 pr-3 pb-3">
            <a href="{{ $item->path() }}" 
                class="block relative border rounded-lg overflow-hidden"
            >
...
<div class="w-full flex flex-wrap mt-6 pb-16">
    @foreach ($categories as $category)
        <div class="w-1/5 mt-3 pr-3">
            <a href="{{ $category->path() }}" 
                class="w-full block p-2 text-bold text-center border rounded hover:bg-blue-100"
            >
                {{ $category->name }}
            </a>
        </div>
