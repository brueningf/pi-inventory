...
<form action="{{ $category->path() }}" method="POST" class="w-4/5 mt-6 mx-auto">
    ...
    <div class="mb-6">
        ...
        <textarea name="description" rows="5" placeholder="e.g. Contains example items">{{ old('description', $category->description) }}</textarea>
    </div>
    ...
    <div class="mb-6">
        ...
        <select name="parent_id" class="bg-white shadow px-3 py-2" required>
            <option {{ old('parent_id') == null ? 'selected' : ''}} disabled>Select a category</option>
