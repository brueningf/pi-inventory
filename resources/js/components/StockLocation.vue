<template>
    <tr>
        <td class="w-1/6">{{ storage.location }}</td>
        <td class="w-1/6">{{ storage.column ? storage.column : '-' }}</td>
        <td class="w-1/6">{{ storage.level ? storage.level : '-' }}</td>
        <td class="w-1/6" v-if="editingStock">
            <form class="flex" @submit.prevent="update">
                ...
                />
                <button class="btn rounded-none bg-green-600 text-white" type="submit">Save</button>
            </form>
        </td>
        <td class="w-1/6" v-else>{{ storage.stock }}</td>
        <td class="w-1/6">{{ storage.status}}</td>
        <td class="w-1/6">
